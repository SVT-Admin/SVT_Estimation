...
function validateStorageData(data) {
  try {
    ...
    if (!data.brands.every((brand) => typeof brand.id === "number" && typeof brand.name === "string")) {
      return false
    }
...
window.addEventListener("DOMContentLoaded", (event) => {
  ...
  if (deleteFromDate && deleteToDate) {
    deleteFromDate.addEventListener("change", updateDeleteBillsCount)
    deleteToDate.addEventListener("change", updateDeleteBillsCount)
  }
})
...
function applyFilters() {
  ...
  if (billNumberFilter) {
    bills = bills.filter((bill) => bill.billNumber.toString() === billNumberFilter)
  }
...
function updateDeleteBillsCount() {
  ...
  const countElement = document.getElementById("delete-bills-count")
